...
constructor(
    ...
    fun insertShoppingItem(name:String,amountString:String,priceString:String){
        ...
        if (name.length>Constants.MAX_NAME_LENGTH){
            _insertShoppingItemStatus.postValue(Event(Resource.error("The name must not exceed ${Constants.MAX_NAME_LENGTH} characters",null)))
            return
        }
        if (priceString.length>Constants.MAX_PRICE_LENGTH){
            _insertShoppingItemStatus.postValue(Event(Resource.error("The price must not exceed ${Constants.MAX_PRICE_LENGTH} ",null)))
            return
        }
        val amount=try {
            amountString.toInt()
